...
import { GetSectionsById, UpdateSectionsById } from "../../../../services/https";
...
function SectionEdit() {
  ...
  const getSection = async (id: string) => {
    ...
    if (res.status === 200) {
      ...
      if (form) {
        (form.elements.namedItem("name") as HTMLSelectElement).value = res.data.name;
        (form.elements.namedItem("name_link") as HTMLInputElement).value = res.data.name_link;
  ...
  const onFinish = async (e: React.FormEvent) => {
    ...
    const res = await UpdateSectionsById(id, values);
    if (res.status === 200) {
    ...
    <div className="section-create-container">
      <div className="form-card">
        <div className="form-header">
          ...
          <h2>แก้ไขเว็บไซต์ที่เกี่ยวข้อง</h2>
          <p>แก้ไขข้อมูลเว็บไซต์ฝ่ายในระบบ</p>
        </div>

        <form id="sectionForm" onSubmit={onFinish} className="user-form">
