...
import { CreateTraining } from "../../../../services/https";
...
function TrainingCreate() {
  ...
  const onFinish = async (e: React.FormEvent) => {
    ...
    try {
      let res = await CreateTraining(formData);
      if (res?.message === "Upload successful") {
        showNotification("success", "สร้างรายการสำเร็จ!");
        setTimeout(() => {
          navigate("/admin/training");
        }, 2000);
      } else {
        console.error("Upload failed:", res);
        showNotification("error", res?.error || "สร้างรายการไม่สำเร็จ");
      }
    } catch (err: any) {
