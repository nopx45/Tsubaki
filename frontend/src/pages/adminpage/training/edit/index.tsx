...
import { GetTrainingsById, UpdateTrainingsById } from "../../../../services/https/index";
...
function TrainingEdit() {
  ...
  const getTrainingsById = async (id: string) => {
    try {
      let res = await GetTrainingsById(id);
      if (res.status === 200) {
        ...
        showNotification("error", "ไม่พบข้อมูลรายการอบรม");
  ...
  const onFinish = async (e: React.FormEvent) => {
    ...
    try {
      let res = await UpdateTrainingsById(id, formData);
      if (res?.data?.message === "Updated successfully" || res?.data?.message === "Upload successful") {
        showNotification("success", "อัปเดตรายการอบรมสำเร็จ!");
        setTimeout(() => navigate("/admin/training"), 2000);
      } else {
        showNotification("error", res?.error || "อัปเดตรายการอบรมไม่สำเร็จ");
      }
    } catch (err: any) {
      ...
      <div className="it-knowledge-edit-card">
        ...
        <form onSubmit={onFinish} className="it-knowledge-form">
          ...
          <div className="form-actions">
            <Link to="/admin/training" className="cancel-button">
