...
const [dateRange, setDateRange] = useState<[dayjs.Dayjs | null, dayjs.Dayjs | null] | null>(null);
  ...
  const [pageVisitors, setPageVisitors] = useState<PageVisitorsInterface[]>([]);
  ...
  const getPageVisitors = async () => {
    try {
      const res = await GetAllPageVisitors();
      if (res.status === 200) {
        setPageVisitors(res.data);
      } else {
  ...
  const deleteVisitor = async (id: string) => {
    ...
    const res = await DeletePageVisitorsById(id);
    if (res.status === 200) {
  ...
  const filtered = pageVisitors.filter((v) => {
    const matchSearch =
    v.username?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    v.page_name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    v.page_path?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    dayjs(v.UpdatedAt).format("DD/MM/YYYY HH:mm:ss").includes(searchTerm);

    const startTime = dayjs(v.UpdatedAt);
  ...
  const handleExportCSV = () => {
    ...
    const headers = ['วันที่', 'ชื่อผู้ใช้', 'ชื่อเพจ', 'พาธ'];
        ...
        <div className="header-section">
          ...
          <div className="header-actions" style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
            <div className="search-container">
              ...
              <input
                type="text"
                placeholder="ค้นหาด้วยชื่อผู้ใช้ / เพจ / วันที่..."
                value={searchTerm}
                ...
                <tbody>
                  {currentData.map((v) => (
                    <tr key={v.ID} className="activity-row">
                      <td>{dayjs(v.UpdatedAt).format("DD/MM/YYYY HH:mm")}</td>
                      <td style={{ color: "#17d632" }}>{v.username}</td>
                      <td style={{ color: "#c19c1c" }}>{v.page_name}</td>
                      <td style={{ color: "#0D47A1" }}>{v.page_path}</td>
                      <td>
                        <div className="action-buttons">
                          <button
                            className="delete-button"
                            onClick={() => deleteVisitor(String(v.ID))}
                          >
                            <FaTrash />
