...
import { CreateKnowledge } from "../../../../services/https";
...
function ITKnowledgeCreate() {
  ...
  const handleGifChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      setPreviewGif(URL.createObjectURL(selectedFile));
    }
  };
  ...
  const onFinish = async (e: React.FormEvent) => {
    ...
    try {
      let res = await CreateKnowledge(formData);
      if (res?.message === "Upload successful") {
        showNotification("success", "สร้างข่าวสารสำเร็จ!");
        setTimeout(() => {
          navigate("/admin/it-knowledge");
        }, 2000);
      } else {
        console.error("Upload failed:", res);
        showNotification("error", res?.error || "สร้างข่าวสารไม่สำเร็จ");
      }
    } catch (err: any) {
      ...
      <div className="it-knowledge-create-card">
        ...
        <form onSubmit={onFinish} className="it-knowledge-form">
          ...
          <div className="form-actions">
            <Link to="/admin/it-knowledge" className="cancel-button">
...
export default ITKnowledgeCreate;